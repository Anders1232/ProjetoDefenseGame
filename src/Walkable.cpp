...
void Walkable::Update(float dt){
	if(controlable){
		...
		if(InputManager::GetInstance().MousePress(SDL_BUTTON_LEFT) &&
			...
			selected = true;
			pathPoints.push_back(new Vec2(PixelToGrid(associated.box.x, associated.box.w),
...
bool Walkable::Is(GameComponentType type) const{
	return type == GameComponentType::WALKABLE;
}
...
void Walkable::AddPath(int x, int y){
	...
	//se o ponto atual for diferente do ultimo ponto colocado (isso deve ser modificado para "se o ponto ainda não foi colocado)
		pathPoints.push_back(new Vec2(PixelToGrid(x, associated.box.w), pathPoints.back()->y ));
	}
	if(pathPoints.back()->y !=
	   PixelToGrid(y, associated.box.h) ){
	//se o ponto atual for diferente do ultimo ponto colocado (isso deve ser modificado para "se o ponto ainda não foi colocado)
		pathPoints.push_back(new Vec2(pathPoints.back()->x, PixelToGrid(y, associated.box.h )));
	}
}
